...
def create_draft(token, title, content, thumb_media_id, author):
    """
    创建草稿 - 修复Unicode编码问题
    关键：使用json.dumps(..., ensure_ascii=False)确保中文不被转义
    """
...
def generate_ai_cover(title, api_key="", provider="zhipu"):
    """
    使用AI生成写实风格封面图
    ...
    Args:
        title: 文章标题
        api_key: API密钥
        provider: 提供商 (zhipu/glm)
    
    Returns:
        封面图文件路径
    """
    if not api_key:
        print("[Warning] 未配置AI API密钥，使用默认封面图")
        return None
    
    print("[AI Cover] 正在生成AI写实风格封面图...")
    
    # 写实风格的AI相关提示词
    prompt = f"""Professional photorealistic cover image for AI technology news article.
    
Subject: Artificial Intelligence and Technology
Style: Photorealistic, cinematic, high-end commercial photography
Scene: Modern AI data center with glowing blue server racks, holographic neural network visualization floating in air, dramatic lighting with blue and purple tones
Elements: 
- Futuristic AI chip or processor with glowing circuits
- Abstract neural network visualization
- Modern tech company office environment
- Blue and purple color scheme
Mood: Professional, cutting-edge, innovative
Quality: 8K, ultra-detailed, professional photography
No text overlay needed"""
    
    try:
        if provider == "zhipu":
            return _generate_with_zhipu(prompt, api_key)
        elif provider == "openai":
            return _generate_with_openai(prompt, api_key)
        else:
            print(f"[Warning] 不支持的AI提供商: {provider}")
            return None
    except Exception as e:
        print(f"[Error] AI封面生成失败: {e}")
        return None


def _generate_with_zhipu(prompt, api_key):
    """使用智谱AI CogView生成写实风格封面"""
    response = requests.post(
    ...
    if response.status_code == 200:
        ...
        if img_response.status_code == 200:
            # 保存原始图像
            output_path = "output/ai_cover_original.png"
            ...
            # 调整大小为微信封面尺寸
            resized_path = _resize_image(output_path, "cover.png")
            return resized_path
...
def _resize_image(input_path, output_path):
    """调整图像大小为微信封面尺寸 (900x383)"""
    try:
...
def main():
    ...
    # 询问是否使用AI生成封面图
    use_ai_cover = input("\n是否使用AI生成写实风格封面图? (y/n, 默认n): ").strip().lower()
    ...
    try:
        ...
        if use_ai_cover == 'y':
            ...
            if zhipu_key:
                ai_cover = generate_ai_cover(article_title, zhipu_key, "zhipu")
            elif openai_key:
                ai_cover = generate_ai_cover(article_title, openai_key, "openai")
            else:
                print("[Warning] 未配置AI API密钥，使用默认封面")
                ai_cover = None
