...
def generate_ai_cover_zimage(title, api_key):
    """使用阿里Z-Image生成写实风格封面图（带代理）"""
    print("[AI Cover] 使用阿里Z-Image生成写实风格封面图...")
    print(f"[Proxy] 使用代理: {PROXY}")
    
    # 写实风格的AI科技提示词 (参考FeiqingqiWechatMP)
    prompt = """Professional photorealistic photograph of a modern AI data center interior.
...
No text overlay needed."""
    ...
    negative_prompt = "blurry, low quality, cartoon, text, watermark, deformed, ugly, overexposed"
...
def main():
    ...
    try:
        ...
        if zimage_api_key:
            ...
            print("[Info] 未配置Z-Image API，使用默认封面图")
            generate_default_cover()
